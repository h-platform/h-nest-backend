...
const topic = "user.resetPasswordByEmail.checkOtp"
const logger = giveMeClassLogger(topic);

export class UserResetPasswordByEmailCheckOtpCommandDTO {
    @ApiProperty()
    @IsDefined()
    otpUuid: string;
...
export class UserResetPasswordByEmailCheckOtpCommand {
    constructor(
        private readonly otpService: OtpService,
        @InjectEntityManager() private readonly manager: EntityManager
    ) { }

    async execute(dto: UserResetPasswordByEmailCheckOtpCommandDTO): Promise<any> {
        logger.debug(`command execution started`)
        logger.silly(`command dto`, dto)
        dto = await transformAndValidate(UserResetPasswordByEmailCheckOtpCommandDTO, dto)
    ...
    @Post(topic)
    async httpHandler(@Body() body: UserResetPasswordByEmailCheckOtpCommandDTO): Promise<any> {
